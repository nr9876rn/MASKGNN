...
contract Rysk is EIP712Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256 public flashLoanRedeemPeriodStart;
  ...
  event MMarketUpdated(address indexed previousMMarket, address indexed newMMarket);
  event FactoryUpdated(address indexed previousFactory, address indexed newFactory);
  event FlashLoanPoolUpdated(address indexed previousPool, address indexed newPool);
  event SwapRouterUpdated(address indexed previousRouter, address indexed newRouter);
  event MarginPoolUpdated(address indexed previousPool, address indexed newPool);
  event FeeRecipientUpdated(address indexed previousRecipient, address indexed newRecipient);
  event GasFeeRecipientUpdated(address indexed previousRecipient, address indexed newRecipient);
  event RyskSignerUpdated(address indexed previousSigner, address indexed newSigner);
  event FlashLoanRedeemPeriodStartUpdated(uint256 previousStart, uint256 newStart);
  event SelfServiceAllowedUpdated(bool allowed);
  event OTCTradeExecuted(address indexed user1, address indexed user2, address asset1, address asset2, uint256 amount1, uint256 amount2, uint64 nonce);
  ...
  string public constant OTC_TRADE_BATCH_TYPE = "OTCTradeBatch(OTCTrade[] trades)OTCTrade(uint256 chainId,address user1,address user2,address asset1,address asset2,uint256 amount1,uint256 amount2,uint64 nonce)";
  ...
  function setOperator(address _operator) external {
    _checkOwner();
    require(_operator != address(0), 'cannot set to zero address');
  ...
  function setController(address _controller) external {
    _checkOwner();
    require(_controller != address(0), 'cannot set to zero address');
    emit ControllerUpdated(address(controller), _controller);
    controller = IController(_controller);
  }

  function setMMarket(address _mmarket) external {
    _checkOwner();
    require(_mmarket != address(0), 'cannot set to zero address');
    emit MMarketUpdated(address(mmarket), _mmarket);
    mmarket = MMarket(_mmarket);
  }

  function setFactory(address _factory) external {
    _checkOwner();
    require(_factory != address(0), 'cannot set to zero address');
  ...
  function setFlashLoanPool(address _flashLoanPool) external {
    _checkOwner();
    require(_flashLoanPool != address(0), 'cannot set to zero address');
    emit FlashLoanPoolUpdated(flashLoanPool, _flashLoanPool);
    flashLoanPool = _flashLoanPool;
  }

  function setSwapRouter(address _swapRouter) external {
    _checkOwner();
    require(_swapRouter != address(0), 'cannot set to zero address');
  ...
  function setMarginPool(address _marginPool) external {
    _checkOwner();
    require(_marginPool != address(0), 'cannot set to zero address');
  ...
  function setGasFeeRecipient(address _gasFeeRecipient) external {
    _checkOwner();
    require(_gasFeeRecipient != address(0), 'cannot set to zero address');
    emit GasFeeRecipientUpdated(gasFeeRecipient, _gasFeeRecipient);
    gasFeeRecipient = _gasFeeRecipient;
  }

  function setRyskSigner(address _ryskSigner) external {
    _checkOwner();
    require(_ryskSigner != address(0), 'cannot set to zero address');
    emit RyskSignerUpdated(ryskSigner, _ryskSigner);
    ryskSigner = _ryskSigner;
  }

  function setFlashLoanRedeemPeriodStart(uint256 _flashLoanRedeemPeriodStart) external {
    _checkOwner();
    emit FlashLoanRedeemPeriodStartUpdated(flashLoanRedeemPeriodStart, _flashLoanRedeemPeriodStart);
  ...
  function setSelfServiceAllowed(bool _selfServiceAllowed) external {
    ...
    emit SelfServiceAllowedUpdated(_selfServiceAllowed);
  }
  ...
  function ingresso_redeem(MMarketOperations.Operation[] memory operations, Actions.ActionArgs[] memory actions) external nonReentrant {
    ...
    require(len == actions.length, "operations and actions length mismatch");
    for (uint256 i = 0; i < len; i++) {
      if(msg.sender != _operator) {
        require(operations[i].user_1 == msg.sender, "only otoken owner can redeem");
      }
      require(operations[i].operationType == MMarketOperations.OperationType.Withdraw, "invalid operations array");
      require(actions[i].actionType == Actions.ActionType.Redeem, "invalid actions array");
      require(operations[i].asset_1 == actions[i].asset, "mmarket withdrawn asset is not redeemed otoken");
      require(operations[i].user_1 == actions[i].secondAddress, "redeem receiver must be otoken owner");
      require(operations[i].amount_1 == actions[i].amount, "redeem amount must be withdraw amount");
      require(operations[i].user_2 == address(this), "invalid withdraw recipient");
    }
  ...
  function ingresso_OTCTrade(bytes calldata payload) external nonReentrant {
    ...
    mmarket.operate(operations);
    emit OTCTradeExecuted(otcTrade.user1, otcTrade.user2, otcTrade.asset1, otcTrade.asset2, otcTrade.amount1, otcTrade.amount2, otcTrade.nonce);
  }

  function ingresso_OTCTradeBatch(bytes calldata payload) external nonReentrant {
    _checkOperator();
    (Parser.OTCTrade[] memory trades, bytes memory signature) = Parser.parseOTCTradeBatch(payload);
    uint256 len = trades.length;

    bytes32 batchDigest = getOTCTradeBatchDigest(trades);
    require(checkSignature(ryskSigner, batchDigest, signature), "invalid OTC Trade batch signature");

    MMarketOperations.Operation[] memory operations = new MMarketOperations.Operation[](len);

    for (uint256 i = 0; i < len; i++) {
      bytes32 tradeDigest = getOTCTradeDigest(trades[i]);
      require(!isDigestUsed[tradeDigest], "OTC trade already used");
      isDigestUsed[tradeDigest] = true;

      operations[i] = MMarketOperations.Operation(
        MMarketOperations.OperationType.ConductTrade,
        trades[i].user1,
        trades[i].user2,
        trades[i].asset1,
      ...
      emit OTCTradeExecuted(trades[i].user1, trades[i].user2, trades[i].asset1, trades[i].asset2, trades[i].amount1, trades[i].amount2, trades[i].nonce);
    }
  ...
  function getOTCTradeBatchDigest(Parser.OTCTrade[] memory trades) public view returns (bytes32) {
    bytes32 tradeTypeHash = keccak256(bytes(OTC_TRADE_TYPE));
    bytes32 batchTypeHash = keccak256(bytes(OTC_TRADE_BATCH_TYPE));

    bytes32[] memory hashes = new bytes32[](trades.length);
...
// ===== FILE: src/MMarketOperations.sol =====
/**
 * SPDX-License-Identifier: UNLICENSED
 */
pragma solidity ^0.8.22;
...
// ===== FILE: lib/openzeppelin-contracts/contracts/interfaces/IERC7913.sol =====
// SPDX-License-Identifier: MIT
