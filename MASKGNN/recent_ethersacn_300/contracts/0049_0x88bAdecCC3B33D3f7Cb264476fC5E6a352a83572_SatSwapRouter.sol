// ===== FILE: contracts/SatSwapRouter.sol =====
...
/// @title SatSwapRouter
/// @notice Minimal router that pre-settles the input currency to the PoolManager and then
///         calls swap with the swapper address encoded into hookData. Use this (or a fork)
///         to buy and sell sat through SatHook.
contract SatSwapRouter is IUnlockCallback {
    ...
    /// @notice Buy SAT with ETH. Pre-settles `msg.value` to the manager, calls swap,
    ///         takes SAT output for `recipient`. `swapper` is encoded into hookData and
    ///         is the address whose buy-block timestamp is recorded on the hook.
    function buy(PoolKey calldata key, address swapper, address recipient)
        external
        payable
        returns (uint256 satut)
    {
        ...
        satut = abi.decode(ret, (uint256));
    }

    /// @notice Sell SAT for ETH. Caller must approve this router for at least `satIn`
    ///         beforehand. `swapper` is recorded on the hook (same-block-as-last-buy
    ///         cooldown applies).
    function sell(PoolKey calldata key, address swapper, address recipient, uint256 satIn)
        external
        returns (uint256 ethOut)
    {
        IERC20Minimal sat = IERC20Minimal(Currency.unwrap(key.currency1));
        if (!sat.transferFrom(msg.sender, address(this), satIn)) revert TransferFailed();
        bytes memory data = abi.encode(Action({
    ...
    function unlockCallback(bytes calldata raw) external returns (bytes memory) {
        ...
        bytes memory hookData = abi.encode(a.swapper);

        if (a.direction == Direction.Buy) {
            manager.settle{value: a.amountIn}();
            BalanceDelta delta = manager.swap(a.key, SwapParams({
                zeroForOne: true,
                amountSpecified: -int256(a.amountIn),
                sqrtPriceLimitX96: TickMath.MIN_SQRT_PRICE + 1
            }), hookData);
            int256 amount1 = delta.amount1();
            uint256 satut = uint256(amount1 > 0 ? amount1 : -amount1);
            manager.take(a.key.currency1, a.recipient, satut);
            return abi.encode(satut);
        } else {
            IERC20Minimal sat = IERC20Minimal(Currency.unwrap(a.key.currency1));
            manager.sync(a.key.currency1);
            if (!sat.transfer(address(manager), a.amountIn)) revert TransferFailed();
            manager.settle();
            BalanceDelta delta = manager.swap(a.key, SwapParams({
                zeroForOne: false,
                amountSpecified: -int256(a.amountIn),
