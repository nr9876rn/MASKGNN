// ===== FILE: src/ethereum/EthArchive1.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13;

import "../types/request.sol";
import "../types/response.sol";
import {ICurveStableReader} from "../common/ICurveStableReader.sol";
import {ICommonReader} from "../common/ICommonReader.sol";
import {IFluidDexReader} from "../common/IFluidDexReader.sol";
import {IUniswapV2LikeReader} from "../common/IUniswapV2LikeReader.sol";
import {IUniswapV3LikeReader} from "../common/IUniswapV3LikeReader.sol";
import {IUniswapV4HookReader} from "../common/IUniswapV4HookReader.sol";
import {IUniswapV4LikeReader} from "../common/IUniswapV4LikeReader.sol";
import {IWombatReader} from "../common/IWombatReader.sol";
import {ILidoReader} from "../common/ILidoReader.sol";
import {IEkuboReader} from "../common/IEkuboReader.sol";
import {IBalancerV2EclpReader} from "../common/IBalancerV2EclpReader.sol";
import {IBalancerV2StableReader} from "../common/IBalancerV2StableReader.sol";
import {IBalancerV2WeightedReader} from "../common/IBalancerV2WeightedReader.sol";

contract EthArchive1 {
    ICommonReader public constant commonReader = ICommonReader(0xF32f5e384c64d329c33a5f31D658b25a199D4cAD);
    IUniswapV2LikeReader public constant uniswapV2Reader = IUniswapV2LikeReader(0x9333Fe88F46d08C8b277c71dB2e58B3620027228);
    IUniswapV3LikeReader public constant uniswapV3Reader = IUniswapV3LikeReader(0x0F2F8CFD3CFFD052f350f7E121FE6151f8c47580);
    IUniswapV4LikeReader public constant uniswapV4Reader = IUniswapV4LikeReader(0xB954B1BD262fef08dEBc020FEBDB89Cb187Cb09F);
    IUniswapV4HookReader public constant uniswapV4HookReader = IUniswapV4HookReader(0xBb96cd1277fdb9E95A12b36E67EC24dA21DF577f);
    IWombatReader public constant wombatReader = IWombatReader(0x033614A6BC0c5B0c602e82871fFb38165729b640);
    IFluidDexReader public constant fluidDexReader = IFluidDexReader(0x870090EE541C22e97F11A07dfee1a5c012B9Def6);
    ICurveStableReader public constant curveStableReader = ICurveStableReader(0x8e4504eDBB8f4De7325c985CDfb614e86A7370A6);
    ILidoReader public constant lidoReader = ILidoReader(0x7fB27518980dEB7F90931C1D00c5237d12616eb9);
    IEkuboReader public constant ekuboReader = IEkuboReader(0x370E4307A7f79210EC3cC74c7D7971E062e17a63);
    IBalancerV2EclpReader public constant balancerV2EclpReader = IBalancerV2EclpReader(0xb66331AE2C260037ad27d2439930610294bFCdFF);
    IBalancerV2StableReader public constant balancerV2StableReader = IBalancerV2StableReader(0x2D439BA863dCE9d6f114B99dF702a0E157feaDD0);
    IBalancerV2WeightedReader public constant balancerV2WeightedReader = IBalancerV2WeightedReader(0x081E9DDf71560c8f0104E23B2E12245CEEe42b3b);

    function getMaxHolder(address token, address[] calldata holders) public view returns (address, uint256) {
        return commonReader.getMaxHolder(token, holders);
    }

    function uniswapV2LikeFactory(address poolAddr) public  returns (address) {
        return uniswapV2Reader.uniswapV2LikeFactory(poolAddr);
    }

    function uniswapV2LikePoolInfo(UniswapV2LikeRequest calldata params) public  returns (UniswapV2LikeResponse memory) {
        return uniswapV2Reader.uniswapV2LikePoolInfo(params);
    }

    function uniswapV3LikeFactory(address poolAddr) public view returns (address) {
        return uniswapV3Reader.uniswapV3LikeFactory(poolAddr);
    }

    function uniswapV3LikePoolInfo(UniswapV3LikeRequest calldata params) public view returns (UniswapV3LikeResponse memory) {
        return uniswapV3Reader.uniswapV3LikePoolInfo(params);
    }

    function uniswapV3LikeTicks(UniswapV3LikeTicksRequest calldata params) public view returns (UniswapV3LikeTicksResponse memory) {
        return uniswapV3Reader.uniswapV3LikeTicks(params);
    }

    function uniswapV4LikePoolInfoV3(UniswapV4LikeV3Request calldata params) external view returns (UniswapV4LikeResponseV2 memory) {
        return uniswapV4Reader.uniswapV4LikePoolInfoV3(params);
    }

    function uniswapV4LikeTicks(UniswapV4LikeTicksRequest calldata params) external view returns (UniswapV4LikeTicksResponse memory) {
        return uniswapV4Reader.uniswapV4LikeTicks(params);
    }

    function clankerHookV2DataV2(ClankerHookV2RequestV2 calldata params) external view returns (ClankerHookV2Response memory) {
        return uniswapV4HookReader.clankerHookV2DataV2(params);
    }

    function nftStrategyHookInfoV2(UniswapV4HookRequest calldata params) external view returns (NftStrategyHookResponse memory){
        return uniswapV4HookReader.nftStrategyHookInfoV2(params);
    }

    function wombatPoolInfo(WombatRequest calldata params) external view returns (WombatResponse memory) {
        return wombatReader.wombatPoolInfo(params);
    }

    function wombatIsDynamic(address poolAddr) external view returns (bool) {
        return wombatReader.isDynamicPool(poolAddr);
    }

    function fluidDexFactory(address poolAddr) external view returns (address) {
        return fluidDexReader.fluidDexFactory(poolAddr);
    }

    function fluidDexSomePoolsAddress(address factory, uint256 maxpools) external view returns (address[] memory pools) {
        return fluidDexReader.fluidDexSomePoolsAddress(factory, maxpools);
    }

    function fluidDexPoolInfo(FluidDexRequest calldata params) external returns (FluidDexResponse memory) {
        return fluidDexReader.fluidDexPoolInfo(params);
    }

    function fluidExtraLiquidity(FluidExtraLiquidityRequest calldata params) external view returns (FluidExtraLiquidityResponse memory) {
        return fluidDexReader.fluidExtraLiquidity(params);
    }

    function curveStableFactoryV2(CurveFactoryRequest calldata params) external view returns (CurveFilterResponseV2 memory ){
        return curveStableReader.curveStableFactoryV2(params);
    }

    function curveFactory(CurveFactoryRequest calldata params) external view returns (address){
        return curveStableReader.curveFactory(params);
    }

    function ethLidoStakePoolInfo(LidoRequest calldata params) external view returns (LidoResponse memory) {
        return lidoReader.ethLidoStakePoolInfo(params);
    }
    ...
    function curveBaseInfo(CurveFactoryRequest calldata params) external view returns (CurveBaseInfoResponse memory response){
        return curveStableReader.curveBaseInfo(params);
    }

    function curveStablePoolInfoV3(CurveFactoryRequest calldata params) external view returns (CurveStableResponseV3 memory ){
        return curveStableReader.curveStablePoolInfoV3(params);
    }

    function curveCryptoPoolInfoV2(CurveFactoryRequest calldata params) external view returns (CurveCryptoResponseV2 memory ){
        return curveStableReader.curveCryptoPoolInfoV2(params);
    }

    function balancerV2EclpPoolInfo(BalancerV2EclpRequest calldata params) external view returns (BalancerV2EclpResponse memory) {
        return balancerV2EclpReader.balancerV2EclpPoolInfo(params);
    }

    function checkEclpPoolType(BalancerV2EclpRequest calldata params) external view returns (BalancerV2EclpPoolTypeResponse memory) {
        return balancerV2EclpReader.checkEclpPoolType(params);
    }

    function balancerV2StablePoolInfo(BalancerV2StableRequest calldata params) external view returns (BalancerV2StableResponse memory) {
        return balancerV2StableReader.balancerV2StablePoolInfo(params);
    }

    function balancerV2WeightedPoolInfo(BalancerV2WeightedRequest calldata params) external view returns (BalancerV2WeightedResponse memory) {
        return balancerV2WeightedReader.balancerV2WeightedPoolInfo(params);
    }

    function detectWeightedPoolType(BalancerV2PoolTypeRequest calldata params) external view returns (BalancerV2PoolTypeResponse memory) {
        return balancerV2WeightedReader.detectWeightedPoolType(params);
    }

    function detectStablePoolType(BalancerV2PoolTypeRequest calldata params) external view returns (BalancerV2PoolTypeResponse memory) {
        return balancerV2StableReader.detectStablePoolType(params);
    }
}
...
// ===== FILE: src/common/ICurveStableReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;
 
import {CurveFactoryRequest} from "../types/request.sol";
import {CurveFilterResponseV2,CurveStableResponseV3,CurveCryptoResponseV2,CurveBaseInfoResponse} from "../types/response.sol";

interface ICurveStableReader {

   function curveBaseInfo(CurveFactoryRequest calldata params) external view returns (CurveBaseInfoResponse memory response);
   //判断pool是否属于curve协议，并返回对应的factory地址
   function curveFactory(CurveFactoryRequest calldata params) external view returns (address);
   //curve stable需要过滤的条件较多，合并一起，不仅返回factory还返回了其他判断条件
   function curveStableFactoryV2(CurveFactoryRequest calldata params) external view returns (CurveFilterResponseV2 memory response);
   //稳定币池子信息
   function curveStablePoolInfoV3(CurveFactoryRequest calldata params) external view returns (CurveStableResponseV3 memory response);
   //非稳定币池子信息
   function curveCryptoPoolInfoV2(CurveFactoryRequest calldata params) external view returns (CurveCryptoResponseV2 memory response);
}

// ===== FILE: src/common/ICommonReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

interface ICommonReader {
    function getMaxHolder(address token, address[] calldata holders) external view returns (address, uint256);
}

// ===== FILE: src/common/IFluidDexReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {FluidDexRequest, FluidExtraLiquidityRequest} from "../types/request.sol";
import {FluidDexResponse, FluidExtraLiquidityResponse} from "../types/response.sol";

interface IFluidDexReader {
    function fluidDexFactory(address poolAddr) external view returns (address);
    function fluidDexSomePoolsAddress(address factory, uint256 maxpools) external view returns (address[] memory pools);
    function fluidDexPoolInfo(FluidDexRequest calldata params) external returns (FluidDexResponse memory response);
    function fluidExtraLiquidity(FluidExtraLiquidityRequest calldata params) external view returns (FluidExtraLiquidityResponse memory response);
}


// ===== FILE: src/common/IUniswapV2LikeReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.4;

import {UniswapV2LikeRequest} from "../types/request.sol";
import {UniswapV2LikeResponse, CamelotV2Response} from "../types/response.sol";

interface IUniswapV2LikeReader {
   /// @notice Identifies the UniswapV2-compatible factory for a given pool address.
   /// @dev `view` is intentionally omitted to avoid STATICCALL propagation.
   ///      Certain non-standard ERC-20 / cToken contracts (e.g. Venus vBNB on BNB Chain)
   ///      declare `token0()` as `view` in their ABI but perform internal state writes at
   ///      the EVM level.  When this reader is invoked under a STATICCALL context (which
   ///      Solidity enforces for `view` functions), those writes trigger an EVM
   ///      "write protection" revert that consumes *all* forwarded gas — causing the
   ///      entire multicall batch to fail with out-of-gas rather than returning a safe
   ///      zero address.
   ///
   ///      Removing `view` ensures the call is dispatched via CALL instead of STATICCALL,
   ///      so the rogue `token0()` write does not raise write-protection.  The function
   ///      itself contains zero storage writes, zero ETH transfers, and emits no events;
   ///      read-only semantics are preserved by convention and enforced off-chain via
   ///      `eth_call`.  All production callers use `eth_call` / multicall simulation and
   ///      never broadcast this as a state-changing transaction.
   function uniswapV2LikeFactory(address poolAddr) external returns (address);

   /// @notice Fetches reserves and token metadata for a UniswapV2-like pool.
   /// @dev `view` is intentionally omitted for the same reason as `uniswapV2LikeFactory`:
   ///      some pool contracts implement `token0()` / `token1()` with internal state writes
   ///      that are incompatible with STATICCALL.  Keeping this as a non-view prevents
   ///      cascading write-protection reverts in multicall batches.
   ///      The implementation contains no storage mutations; callers MUST invoke this
   ///      function via `eth_call` (or equivalent read-only simulation) only.
   function uniswapV2LikePoolInfo(UniswapV2LikeRequest calldata params) external returns (UniswapV2LikeResponse memory response);
   function camelotV2PoolInfo(UniswapV2LikeRequest calldata params) external view returns (CamelotV2Response memory response);
   function hydrexV2Factory(address poolAddr, address factoryAddr) external view returns (address factory, bool stable);   
}
...
// ===== FILE: src/common/IUniswapV4HookReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {ClankerHookV2RequestV2, UniswapV4HookRequest} from "../types/request.sol";
import {ClankerHookV2Response, NftStrategyHookResponse, FlaunchHookResponse, DopplerHookInitializerResponse, ClankerHookResponse, ZoraV4HookResponse, AlphixHookResponse} from "../types/response.sol";

interface IUniswapV4HookReader {
    function clankerHookV2DataV2(ClankerHookV2RequestV2 calldata params) external view returns (ClankerHookV2Response memory);
    function clankerHookData(ClankerHookV2RequestV2 calldata params) external view returns (ClankerHookResponse memory response);
    function nftStrategyHookInfoV2(UniswapV4HookRequest calldata params) external view returns (NftStrategyHookResponse memory response);
    function flaunchHookDataV2(UniswapV4HookRequest calldata params) external view returns (FlaunchHookResponse memory response);
    function dopplerHookInitializerData(UniswapV4HookRequest calldata params) external view returns (DopplerHookInitializerResponse memory response);
    function getZoraV4HookData(UniswapV4HookRequest calldata params) external view returns (ZoraV4HookResponse memory response);
    function alphixHookData(UniswapV4HookRequest calldata params) external view returns (AlphixHookResponse memory response);
}

// ===== FILE: src/common/IUniswapV4LikeReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {UniswapV4LikeRequest, UniswapV4LikeTicksRequest, PancakeInfBinRequest, PancakeInfBinBinsRequest,UniswapV4LikeV3Request} from "../types/request.sol";
import {UniswapV4LikeResponse, UniswapV4LikeResponseV2, PancakeInfClResponse, PancakeInfClResponseV2, UniswapV4LikeTicksResponse, 
    PancakeInfBinResponse, PancakeInfBinResponseV2, PancakeInfBinBinsResponse} from "../types/response.sol";
interface IUniswapV4LikeReader {
    function uniswapV4LikePoolInfoV2(UniswapV4LikeRequest calldata params) external view returns (UniswapV4LikeResponseV2 memory);
    function uniswapV4LikePoolInfoV3(UniswapV4LikeV3Request calldata params) external view returns (UniswapV4LikeResponseV2 memory);
    function uniswapV4LikeTicks(UniswapV4LikeTicksRequest calldata params) external view returns (UniswapV4LikeTicksResponse memory);
    function pancakeInfClPoolInfo(UniswapV4LikeRequest calldata params) external view returns (PancakeInfClResponse memory);
    function pancakeInfClPoolInfoV2(UniswapV4LikeRequest calldata params) external view returns (PancakeInfClResponseV2 memory);
    function pancakeInfBinPoolInfo(PancakeInfBinRequest calldata params) external view returns (PancakeInfBinResponse memory);
    function pancakeInfBinPoolInfoV2(PancakeInfBinRequest calldata params) external view returns (PancakeInfBinResponseV2 memory);
    function pancakeInfBinBins(PancakeInfBinBinsRequest calldata params) external view returns (PancakeInfBinBinsResponse memory);
}

// ===== FILE: src/common/IWombatReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {WombatRequest} from "../types/request.sol";
import {WombatResponse} from "../types/response.sol";

interface IWombatReader {
    function wombatPoolInfo(WombatRequest calldata params) external view returns (WombatResponse memory);
    function isDynamicPool(address pool) external view returns (bool);
}

// ===== FILE: src/common/ILidoReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {LidoRequest} from "../types/request.sol";
import {LidoResponse} from "../types/response.sol";

interface ILidoReader {
    function ethLidoStakePoolInfo(LidoRequest calldata params) external view returns (LidoResponse memory response);
}
...
// ===== FILE: src/common/IBalancerV2EclpReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {BalancerV2EclpRequest} from "../types/request.sol";
import {BalancerV2EclpResponse, BalancerV2EclpPoolTypeResponse} from "../types/response.sol";

interface IBalancerV2EclpReader {
    function balancerV2EclpPoolInfo(BalancerV2EclpRequest calldata params) 
        external 
        view 
        returns (BalancerV2EclpResponse memory response);

    function checkEclpPoolType(BalancerV2EclpRequest calldata params)
        external
        view
        returns (BalancerV2EclpPoolTypeResponse memory response);
}



// ===== FILE: src/common/IBalancerV2StableReader.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import {BalancerV2StableRequest, BalancerV2PoolTypeRequest} from "../types/request.sol";
import {BalancerV2StableResponse, BalancerV2PoolTypeResponse} from "../types/response.sol";

interface IBalancerV2StableReader {
    function balancerV2StablePoolInfo(BalancerV2StableRequest calldata params)
        external
        view
        returns (BalancerV2StableResponse memory response);

    function detectStablePoolType(BalancerV2PoolTypeRequest calldata params)
        external
        view
        returns (BalancerV2PoolTypeResponse memory response);
}



// ===== FILE: src/common/IBalancerV2WeightedReader.sol =====
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import {BalancerV2WeightedRequest, BalancerV2PoolTypeRequest} from "../types/request.sol";
import {BalancerV2WeightedResponse, BalancerV2PoolTypeResponse} from "../types/response.sol";

interface IBalancerV2WeightedReader {
    function balancerV2WeightedPoolInfo(BalancerV2WeightedRequest calldata params) 
        external 
        view 
        returns (BalancerV2WeightedResponse memory response);

    function detectWeightedPoolType(BalancerV2PoolTypeRequest calldata params)
        external
        view
        returns (BalancerV2PoolTypeResponse memory response);
}
