// ===== FILE: SyrupRouter.sol =====
// SPDX-License-Identifier: BUSL-1.1
pragma solidity =0.8.7 ^0.8.0 ^0.8.7;

// modules/syrup-utils/modules/erc20-helper/src/interfaces/IERC20Like.sol
...
// modules/syrup-utils/contracts/interfaces/ISyrupRouter.sol
...
// modules/syrup-utils/contracts/interfaces/Interfaces.sol
...
interface IGlobalsLike {
    ...
    function isInstanceOf(bytes32 key, address instance) external view returns (bool isInstanceOf);
...
interface IPoolLike is IERC20Like_1 {
...
interface IProxyFactoryLike {
    ...
    function isInstance(address instance) external view returns (bool isInstance);
...
// modules/syrup-utils/modules/erc20-helper/src/ERC20Helper.sol
...
library ERC20Helper {
    ...
    function transfer(address token_, address to_, uint256 amount_) internal returns (bool success_) {
        return _call(token_, abi.encodeWithSelector(IERC20Like_0.transfer.selector, to_, amount_));
    }

    function transferFrom(address token_, address from_, address to_, uint256 amount_) internal returns (bool success_) {
        return _call(token_, abi.encodeWithSelector(IERC20Like_0.transferFrom.selector, from_, to_, amount_));
    }

    function approve(address token_, address spender_, uint256 amount_) internal returns (bool success_) {
        // If setting approval to zero fails, return false.
        if (!_call(token_, abi.encodeWithSelector(IERC20Like_0.approve.selector, spender_, uint256(0)))) return false;

        // If `amount_` is zero, return true as the previous step already did this.
        if (amount_ == uint256(0)) return true;

        // Return the result of setting the approval to `amount_`.
        return _call(token_, abi.encodeWithSelector(IERC20Like_0.approve.selector, spender_, amount_));
    }
...
// modules/syrup-utils/contracts/SyrupRouter.sol
...
contract SyrupRouter is ISyrupRouter {
    ...
    function _permit(address asset_, uint256 deadline_, uint256 amount_, uint8 v_, bytes32 r_, bytes32 s_) internal {
        uint256 allowance_ = IERC20Like_1(asset_).allowance(msg.sender, address(this));

        if (allowance_ < amount_) {
            IERC20Like_1(asset_).permit(msg.sender, address(this), amount_, deadline_, v_, r_, s_);
        }
    }
