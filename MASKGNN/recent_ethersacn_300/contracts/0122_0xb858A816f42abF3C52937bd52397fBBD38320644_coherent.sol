// ===== FILE: _limitbreak/creator-token-standards/src/access/OwnableBasic.sol =====
...
// ===== FILE: _limitbreak/creator-token-standards/src/access/OwnablePermissions.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/erc721c/ERC721C.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/interfaces/ICreatorToken.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/interfaces/ICreatorTokenLegacy.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/interfaces/ITransferValidator.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/interfaces/ITransferValidatorSetTokenType.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/programmable-royalties/BasicRoyalties.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/token/erc721/ERC721OpenZeppelin.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: _limitbreak/creator-token-standards/src/utils/AutomaticValidatorTransferApproval.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/utils/CreatorTokenBase.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/creator-token-standards/src/utils/TransferValidation.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _limitbreak/permit-c/src/Constants.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: _openzeppelin/contracts/token/common/ERC2981.sol =====
// SPDX-License-Identifier: MIT
...
contract coherent is ERC721C, BasicRoyalties, OwnableBasic {
    ...
    event FreeMintClaimed(address indexed minter, uint256 tokenId);
    ...
    string public constant NAME = "Coherent";
    string public constant SYMBOL = "Coherent";

    bool public freeMintActive = true;

    constructor(
    ) ERC721OpenZeppelin(NAME, SYMBOL) OwnableBasic() BasicRoyalties(msg.sender, 500) {
        _currentIndex = 1;
    }
    ...
    function freeMint() external noContracts {
        ...
        if (hasFreeMinted[msg.sender]) revert AlreadyFreeMinted();
        if (_currentIndex > MAX_SUPPLY) revert ExceedsMaxSupply();
    ...
    function mint(uint256 quantity) external payable noContracts {
        ...
        if (startTokenId + quantity - 1 > MAX_SUPPLY) revert ExceedsMaxSupply();
        if (msg.value < COST * quantity) revert InsufficientPayment();
    ...
    /// @notice Returns a base64-encoded data URI containing JSON metadata with an on-chain SVG image.
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        // Seed all randomness from tokenId + contract address for uniqueness
        uint256 seed = uint256(keccak256(abi.encodePacked(tokenId, address(this))));

        string memory svg = _generateSVG(seed, tokenId);
        string memory svgBase64 = _base64Encode(bytes(svg));

        // Number of blobs for metadata attribute
        uint256 blobCount = 3 + (seed % 4); // 3-6

        string memory json = string(abi.encodePacked(
            '{"name":"Coherent #', _toString(tokenId),
            unicode'","description":"COHERENT","image":"data:image/svg+xml;base64,',
            svgBase64,
            '","attributes":[{"trait_type":"Blobs","value":', _toString(blobCount),
    ...
    // 10% border: blobs are drawn in the inner 800x800 area offset by 100px on each side
    uint256 private constant BORDER = 100; // 10% of 1000
    uint256 private constant INNER  = 800; // CANVAS - 2 * BORDER

    /// @dev Generates the full SVG string for a token.
    function _generateSVG(uint256 seed, uint256 tokenId) internal pure returns (string memory) {
        // Background color derived from seed
        string memory bgColor = "dddddd";

        // Number of blobs: 3-6
        uint256 blobCount = 3 + (seed % 4);

        string memory blobs = "";
        for (uint256 i = 0; i < blobCount; i++) {
            blobs = string(abi.encodePacked(blobs, _generateBlob(seed, i)));
        }

        return string(abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000">',
            '<rect width="1000" height="1000" fill="#', bgColor, '"/>',
            // White border frame (10% on each side)
            '<rect x="100" y="100" width="800" height="800" fill="none" stroke="#dddddd" stroke-width="0"/>',
            // Inner content area clipped to the 800x800 region
            '<defs>',
            '<clipPath id="inner"><rect x="100" y="100" width="800" height="800"/></clipPath>',
            '</defs>',
            // White border strips (top, bottom, left, right — each 10% wide)
            '<rect x="0"   y="0"   width="1000" height="100"  fill="#dddddd"/>',
            '<rect x="0"   y="900" width="1000" height="100"  fill="#dddddd"/>',
            '<rect x="0"   y="0"   width="100"  height="1000" fill="#dddddd"/>',
            '<rect x="900" y="0"   width="100"  height="1000" fill="#dddddd"/>',
            '<g clip-path="url(#inner)">',
            blobs,
            '</g>',
            // Token ID watermark, bottom-right (inside the inner area)
            '<text x="880" y="888" font-family="monospace" font-size="18" fill="rgba(255,255,255,0.35)" text-anchor="end">rare #', _toString(tokenId), '</text>',
            '</svg>'
        ));
    }

    /// @dev Generates one blob ellipse element.
    /// Blob positions are constrained to the inner 800x800 area (offset by BORDER=100).
    function _generateBlob(uint256 seed, uint256 index) internal pure returns (string memory) {
        uint256 h0 = uint256(keccak256(abi.encodePacked(seed, index, uint256(0))));
        uint256 h1 = uint256(keccak256(abi.encodePacked(seed, index, uint256(1))));
        uint256 h2 = uint256(keccak256(abi.encodePacked(seed, index, uint256(2))));
        uint256 h3 = uint256(keccak256(abi.encodePacked(seed, index, uint256(3))));
        uint256 h4 = uint256(keccak256(abi.encodePacked(seed, index, uint256(4))));

        BlobParams memory p;
        p.rx  = 100 + (h0 % 280); // slightly smaller max to fit within inner area
        p.ry  = 100 + (h1 % 280);
        // cx/cy offset by BORDER (100) and capped within INNER (800)
        p.cx  = 100 + 50 + (h2 % 700); // 150..850
        p.cy  = 100 + 50 + (h3 % 700); // 150..850
        p.rot = h4  % 360;
        p.color = _randomHex(h0 ^ h3, index + 1);
        p.opInt = 55 + (h1 % 36);

        return string(abi.encodePacked(_blobA(p), _blobB(p)));
    }
    ...
    function _blobA(BlobParams memory p) private pure returns (string memory) {
        return string(abi.encodePacked(
            '<ellipse cx="', _toString(p.cx),
            '" cy="',  _toString(p.cy),
            '" rx="',  _toString(p.rx),
            '" ry="',  _toString(p.ry),
            '" fill="#', p.color,
            '" opacity="0.', _toString(p.opInt),
            '" transform="rotate(', _toString(p.rot), ',', _toString(p.cx), ',', _toString(p.cy), ')"/>'
        ));
    }

    function _blobB(BlobParams memory p) private pure returns (string memory) {
        uint256 cx2  = p.cx + 20;
        uint256 cy2  = p.cy > 15 ? p.cy - 15 : 0;
        uint256 rx2  = p.rx * 85 / 100;
        uint256 ry2  = p.ry * 110 / 100;
        uint256 rot2 = (p.rot + 30) % 360;
        uint256 op2  = p.opInt > 20 ? p.opInt - 20 : 10;
        return string(abi.encodePacked(
            '<ellipse cx="', _toString(cx2),
            '" cy="',  _toString(cy2),
            '" rx="',  _toString(rx2),
            '" ry="',  _toString(ry2),
            '" fill="#', p.color,
            '" opacity="0.', _toString(op2),
            '" transform="rotate(', _toString(rot2), ',', _toString(cx2), ',', _toString(cy2), ')"/>'
        ));
    }

    /// @dev Derives a 6-character hex color string from a hash and a slot index.
    function _randomHex(uint256 hash, uint256 slot) internal pure returns (string memory) {
        uint256 mixed = uint256(keccak256(abi.encodePacked(hash, slot)));
        uint8 r = uint8(mixed & 0xFF);
        uint8 g = uint8((mixed >> 8) & 0xFF);
        uint8 b = uint8((mixed >> 16) & 0xFF);
        return string(abi.encodePacked(_byteToHex(r), _byteToHex(g), _byteToHex(b)));
    }
