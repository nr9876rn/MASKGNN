...
import "creator-token-standards/src/erc721c/ERC721AC.sol";
import "creator-token-standards/src/programmable-royalties/BasicRoyalties.sol";
...
/**
 * @title  QRYPT
 * @notice 8,888 ancient souls. Trapped in signal. Waiting to be found.
 *
 *  Mint rules
 *  ──────────
 *  · Price: 0.0005 ETH per soul (flat, no free tier)
 *  · Hard cap: 2 per wallet (on-chain)
 *  · Rate limit: 20 mints per block (~12 s window)
 *  · Quantity 1 or 2 per transaction
 *
 *  Royalties
 *  ─────────
 *  · 8.88 % (888 bps) enforced via ERC721C Transfer Validator
 *  · ERC-2981 compatible
 */
contract QRYPT is ERC721AC, BasicRoyalties, Ownable, ReentrancyGuard {
    using Strings for uint256;

    // ─── Constants ────────────────────────────────────────────────────────────
    uint256 public constant MAX_SUPPLY  = 8_888;
    uint256 public constant MINT_PRICE  = 0.0005 ether;
    uint256 public constant BLOCK_CAP   = 20;
    uint8   public constant WALLET_CAP  = 2;
    uint96  public constant ROYALTY_BPS = 888; // 8.88%
    ...
    /// @dev Per-wallet mint counter: 0, 1, or 2
    mapping(address => uint8) public mintCount;

    /// @dev Mints taken in a given block — resets automatically each block
    mapping(uint256 => uint256) public mintsPerBlock;

    // ─── Events ───────────────────────────────────────────────────────────────
    event SoulAnchored(address indexed to, uint256 indexed tokenId, uint256 blockNumber);
    event MintToggled(bool active);
    event Withdrawn(address indexed to, uint256 amount);
    event BaseURIUpdated(string uri);

    // ─── Constructor ──────────────────────────────────────────────────────────
    constructor(address royaltyReceiver_, string memory baseURI_)
        ERC721AC("QRYPT", "QRYPT")
    ...
    /**
     * @notice Anchor 1 or 2 souls. Each soul costs MINT_PRICE (0.0005 ETH).
     * @param quantity 1 or 2
     */
    function mint(uint256 quantity) external payable nonReentrant {
        require(mintActive,                                       "Mint not active");
        require(quantity == 1 || quantity == 2,                  "Quantity must be 1 or 2");

        uint256 current = mintCount[msg.sender];
        require(current + quantity <= WALLET_CAP,                "Wallet cap exceeded");
        require(_totalMinted() + quantity <= MAX_SUPPLY,         "Exceeds max supply");
        require(
            mintsPerBlock[block.number] + quantity <= BLOCK_CAP,
            "Block cap reached - wait for next block"
        );
        ...
        mintCount[msg.sender]        = uint8(current + quantity);
        mintsPerBlock[block.number] += quantity;

        uint256 startId = _nextTokenId();
        _mint(msg.sender, quantity);

        for (uint256 i; i < quantity; ++i) {
            emit SoulAnchored(msg.sender, startId + i, block.number);
        }
    ...
    /// @notice Reserve souls for team / pre-anchored supply. Bypasses all caps.
    ...
    /// @notice Update the IPFS base URI (reveal or CID migration).
    function setBaseURI(string calldata uri) external onlyOwner {
        _baseTokenURI = uri;
        emit BaseURIUpdated(uri);
    }

    /// @notice ERC-7572 / OpenSea collection-level metadata URI.
    function contractURI() public view returns (string memory) {
        return _contractURI;
    }

    /// @notice Update the collection-level metadata URI.
    function setContractURI(string calldata uri) external onlyOwner {
        _contractURI = uri;
    }

    /// @notice Update royalty receiver and / or basis points.
    ...
    /// @dev Token IDs start at 1 to match soul numbering (#0001 – #8888).
    function _startTokenId() internal pure override returns (uint256) {
        return 1;
    }

    /// @dev Bridges OZ Ownable with Limit Break's OwnablePermissions requirement.
...
// ===== FILE: lib/creator-token-standards/src/erc721c/ERC721AC.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: lib/creator-token-standards/src/programmable-royalties/BasicRoyalties.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/utils/AutomaticValidatorTransferApproval.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/utils/CreatorTokenBase.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/lib/ERC721A/contracts/ERC721A.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: lib/creator-token-standards/lib/PermitC/src/Constants.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/access/OwnablePermissions.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/interfaces/ICreatorToken.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/interfaces/ICreatorTokenLegacy.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/interfaces/ITransferValidator.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/utils/TransferValidation.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/src/interfaces/ITransferValidatorSetTokenType.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: lib/creator-token-standards/lib/ERC721A/contracts/IERC721A.sol =====
// SPDX-License-Identifier: MIT
// ERC721A Contracts v4.2.3
// Creator: Chiru Labs
