// ===== FILE: src/WCupNFT.sol =====
// SPDX-License-Identifier: MIT
pragma solidity 0.8.17;

import { ERC721SeaDrop } from "seadrop/ERC721SeaDrop.sol";

/// @title  WCupNFT — World Cup Monopoly NFT (SeaDrop compatible)
/// @notice 10000 supply, 3-tier game. Mint flow goes through SeaDrop
///         (0x00005EA00Ac477B1030CE78506496e8C2dE24bf5) so the NFT shows
///         a Mint button on its OpenSea collection page.
///
///         Tier, daily roll usage, position on the 40-square board and
///         pendingRewards are all tracked in the off-chain web2 backend
///         (FastAPI + SQLite). The NFT contract is intentionally minimal:
///         only ownership, royalties and SeaDrop mint plumbing live on chain.
///
///         Royalty (3%) and maxSupply (10000) are configured via the
///         inherited setters in the Deploy script (setRoyaltyInfo /
///         setMaxSupply / updatePublicDrop / setBaseURI / setContractURI).
contract WCupNFT is ERC721SeaDrop {
    constructor(
        string memory name,
