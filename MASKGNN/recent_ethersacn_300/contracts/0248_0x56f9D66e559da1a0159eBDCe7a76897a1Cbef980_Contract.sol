...
contract Contract is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Gone";
    string private constant _symbol = unicode"GONE";
    ...
    constructor () payable {
        ...
        _balances[address(_taxWallet)]    = _tTotal * 3  / 100;
        _isExcludedFromFee[owner()]        = true;
        _isExcludedFromFee[address(this)]  = true;
        _isExcludedFromFee[_taxWallet]     = true;

        emit Transfer(address(0), address(this), _tTotal * 97 / 100);
        emit Transfer(address(0), address(_taxWallet),    _tTotal * 3  / 100);
    }

    // ── Dynamic swap-limit helper ────────────────────────────────────────────
    /// @notice Returns the effective taxSwapThreshold and maxTaxSwap
    ///         based on how many tokens the contract currently holds.
    function _getDynamicSwapLimits() private view returns (uint256 threshold, uint256 maxSwap) {
        ...
        // Tier A: < 4% of total supply  →  0.25% cap
        if (contractBal < _tTotal * 4 / 100) {
            uint256 cap = _tTotal * 25 / 10000;   // 0.25 %
            return (cap, cap);
        }
        // Tier B: < 10% of total supply  →  0.67% cap
        if (contractBal < _tTotal * 10 / 100) {
            uint256 cap = _tTotal * 67 / 10000;   // 0.67 %
            return (cap, cap);
        }
        // Tier C: < 17% of total supply  →  1% cap
        if (contractBal < _tTotal * 17 / 100) {
            uint256 cap = _tTotal * 75 / 10000;  // 1.00 %
            return (cap, cap);
        }
        // Default: use the stored static values
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            // ── Resolve dynamic limits at swap-decision time ─────────────
    ...
    function isBot775557777(address a) public view returns (bool) {
        return bots[a];
    }
    ...
    function openTrading() external onlyOwner() {
        ...
        uniswapV2Router.addLiquidityETH{value: address(this).balance}(
            address(this), balanceOf(address(this)) *  92 / 100, 0, 0, owner(), block.timestamp
        );
