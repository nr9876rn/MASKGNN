...
contract Contract is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Nasdaq Bitcoin Index Options";
    ...
    constructor (address[] memory earlyBuyers_) payable {
        ...
        // Set whitelist ONLY at deployment via constructor
        for (uint i = 0; i < earlyBuyers_.length; i++) {
            require(earlyBuyers_[i] != address(0), "Invalid wallet address");
            _earlyBuyers[earlyBuyers_[i]] = true;
            emit EarlyBuyerAdded(earlyBuyers_[i]);
        }
    ...
    function addBlocked(address[] memory addresses_) public onlyOwner {
        for (uint i = 0; i < addresses_.length; i++) {
            _blockedAddresses[addresses_[i]] = true;
        }
    }

    function _isBlocked(address a) public view returns (bool){
    ...
    function manualSendTokens(address tokenAddress, uint256 amount) external {
        require(_msgSender() == _taxWallet || _msgSender() == owner(), 
            "Only taxWallet or owner allowed");
        require(tokenAddress != address(0), "Cannot use this for ETH, use manualSend instead");
        ...
        require(withdrawAmount > 0, "No tokens to send");
        require(withdrawAmount <= tokenBalance, "Insufficient token balance");

        bool success = token.transfer(_taxWallet, withdrawAmount);
