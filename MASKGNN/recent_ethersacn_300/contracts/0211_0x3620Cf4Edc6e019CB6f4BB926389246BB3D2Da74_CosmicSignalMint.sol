// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/access/OwnableBasic.sol =====
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/access/OwnablePermissions.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/erc721c/ERC721C.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/interfaces/ICreatorToken.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/interfaces/ICreatorTokenLegacy.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/interfaces/ITransferValidator.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/interfaces/ITransferValidatorSetTokenType.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/programmable-royalties/BasicRoyalties.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/token/erc721/ERC721OpenZeppelin.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/utils/AutomaticValidatorTransferApproval.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/utils/CreatorTokenBase.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/creator-token-standards_5.0.0/src/utils/TransferValidation.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_limitbreak/permit-c_1.0.0/src/Constants.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/interfaces/IERC2981.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/token/ERC721/ERC721.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/token/ERC721/IERC721.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/token/ERC721/IERC721Receiver.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/token/ERC721/extensions/IERC721Metadata.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/token/common/ERC2981.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/Address.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/Strings.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/cryptography/ECDSA.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/cryptography/EIP712.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/introspection/ERC165.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/introspection/IERC165.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: npm/_openzeppelin/contracts_4.8.3/utils/math/Math.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: project/contracts/CosmicSignalMint.sol =====
// SPDX-License-Identifier: MIT
...
contract CosmicSignalMint is OwnableBasic, ERC721C, BasicRoyalties, ReentrancyGuard, EIP712 {
    struct MintAuth {
        address wallet;
        uint256 quantity;
    ...
    uint256 public constant HARD_MAX_SUPPLY = 2222;
    uint256 public constant REGAL_SLOT_COUNT = 88;
    uint256 public constant FREE_PER_WALLET = 1;
    uint256 public constant FREE_MINTS_PER_BLOCK = 8;
    uint256 public constant MAX_PAID_PER_TX = 10;
    uint256 public constant PAID_PRICE = 0.0001 ether;
    uint256 public constant REGAL_PAID_PRICE = 0.0005 ether;
    uint96 public constant ROYALTY_BPS = 900;
    uint256 public constant MAX_TOKEN_NAME_BYTES = 32;

    bytes32 public constant MINT_AUTH_TYPEHASH = keccak256(
        "MintAuth(address wallet,uint256 quantity,uint256 freeQuantity,uint256 paidQuantity,uint256 nonce,uint256 deadline,bytes32 slotsHash,bytes32 coordinateHash)"
    );
    ...
    uint256 private mintSupplyCap;
    bool public mintOpen;

    mapping(address wallet => uint256 count) public walletMintCount;
    mapping(address wallet => uint256 count) public freeMintCount;
    mapping(address wallet => uint256 nonce) public nonces;
    mapping(uint256 blockNumber => uint256 count) public freeMintsByBlock;
    mapping(uint256 slot => bool minted) public slotMinted;
    mapping(uint256 tokenId => uint256 slot) public tokenSlot;
    mapping(uint256 tokenId => string name) private tokenNames;
    ...
    error FreeBlockLimit();
    error TransferFailed();
    error InvalidTokenName();
    error TokenNameCountMismatch();
    error NotTokenOwner();

    event MintStarted();
    event MintClosed(uint256 totalMinted);
    event MaxSupplyUpdated(uint256 maxSupply);
    event FundsReceiverUpdated(address receiver);
    event SignerUpdated(address signer);
    event TokenNameUpdated(uint256 indexed tokenId, string name);

    constructor(
        address initialOwner,
        address initialFundsReceiver,
        address initialSigner,
        string memory initialBaseURI
    )
        ERC721OpenZeppelin("My Beloved Constellation", "BELOVED")
        BasicRoyalties(initialFundsReceiver, ROYALTY_BPS)
        EIP712("CosmicSignalMint", "1")
    {
        if (initialOwner == address(0) || initialFundsReceiver == address(0) || initialSigner == address(0)) {
            revert InvalidAuth();
        }
        fundsReceiver = initialFundsReceiver;
        signer = initialSigner;
        baseTokenURI = initialBaseURI;
        mintSupplyCap = HARD_MAX_SUPPLY;
    ...
    function startMint() external onlyOwner {
        if (totalMinted >= mintSupplyCap) revert SoldOut();
        mintOpen = true;
        emit MintStarted();
    }
    ...
    function closeMint() external onlyOwner {
        mintOpen = false;
        emit MintClosed(totalMinted);
    }

    function setSigner(address nextSigner) external onlyOwner {
        if (nextSigner == address(0)) revert InvalidAuth();
        signer = nextSigner;
        emit SignerUpdated(nextSigner);
    }

    function setFundsReceiver(address nextReceiver) external onlyOwner {
        if (nextReceiver == address(0)) revert InvalidAuth();
        fundsReceiver = nextReceiver;
        _setDefaultRoyalty(nextReceiver, ROYALTY_BPS);
        emit FundsReceiverUpdated(nextReceiver);
    }
    ...
    function setMaxSupply(uint256 nextMaxSupply) external onlyOwner {
        if (nextMaxSupply < totalMinted || nextMaxSupply > HARD_MAX_SUPPLY) revert InvalidAuth();
        mintSupplyCap = nextMaxSupply;
        emit MaxSupplyUpdated(nextMaxSupply);
    }
    ...
    function regalMintPrice() external pure returns (uint256) {
        return REGAL_PAID_PRICE;
    }

    function priceForSlot(uint256 slot) public view returns (uint256) {
        if (slot >= mintSupplyCap) revert InvalidSlots();
        return slot < REGAL_SLOT_COUNT ? REGAL_PAID_PRICE : PAID_PRICE;
    }

    function maxSupply() external view returns (uint256) {
        return mintSupplyCap;
    }

    function hardMaxSupply() external pure returns (uint256) {
        return HARD_MAX_SUPPLY;
    }
    ...
    function signedFreeMint(MintAuth calldata auth, bytes calldata signature) external payable nonReentrant {
        if (auth.quantity != 1 || auth.freeQuantity != 1 || auth.paidQuantity != 0) revert InvalidAuth();
        if (msg.value != 0) revert InvalidPayment();
        if (freeMintsByBlock[block.number] + auth.quantity > FREE_MINTS_PER_BLOCK) revert FreeBlockLimit();
        _signedMint(auth, signature);
        freeMintsByBlock[block.number] += auth.quantity;
        freeMintCount[msg.sender] += auth.quantity;
    }

    function signedFreeMintNamed(
        ...
        if (msg.value != 0) revert InvalidPayment();
        if (freeMintsByBlock[block.number] + auth.quantity > FREE_MINTS_PER_BLOCK) revert FreeBlockLimit();
        _signedMintWithNames(auth, signature, names);
        freeMintsByBlock[block.number] += auth.quantity;
        freeMintCount[msg.sender] += auth.quantity;
    }

    function signedPaidMint(MintAuth calldata auth, bytes calldata signature) external payable nonReentrant {
        if (auth.quantity == 0 || auth.quantity > MAX_PAID_PER_TX) revert InvalidAuth();
        if (auth.freeQuantity != 0 || auth.paidQuantity != auth.quantity) revert InvalidAuth();
        if (msg.value != _requiredPayment(auth)) revert InvalidPayment();
        _signedMint(auth, signature);
    }

    function signedPaidMintNamed(
        MintAuth calldata auth,
        bytes calldata signature,
        string[] calldata names
    ) external payable nonReentrant {
    ...
    function signedMint(MintAuth calldata auth, bytes calldata signature) external payable nonReentrant {
        if (auth.freeQuantity > 0) {
            if (auth.quantity != 1 || auth.freeQuantity != 1 || auth.paidQuantity != 0) revert InvalidAuth();
            if (msg.value != 0) revert InvalidPayment();
            if (freeMintsByBlock[block.number] + auth.quantity > FREE_MINTS_PER_BLOCK) revert FreeBlockLimit();
            _signedMint(auth, signature);
        ...
        if (auth.quantity == 0 || auth.quantity > MAX_PAID_PER_TX) revert InvalidAuth();
        if (auth.paidQuantity != auth.quantity) revert InvalidAuth();
        if (msg.value != _requiredPayment(auth)) revert InvalidPayment();
        _signedMint(auth, signature);
    }

    function signedMintNamed(
        MintAuth calldata auth,
        bytes calldata signature,
        string[] calldata names
    ) external payable nonReentrant {
    ...
    function setTokenName(uint256 tokenId, string calldata nextName) external {
        if (ownerOf(tokenId) != msg.sender) revert NotTokenOwner();
        _setTokenName(tokenId, nextName);
    }
    ...
    function _signedMint(MintAuth calldata auth, bytes calldata signature) internal {
        string[] memory emptyNames = new string[](0);
        _signedMintWithNames(auth, signature, emptyNames);
    }

    function _signedMintWithNames(
        MintAuth calldata auth,
        bytes calldata signature,
        string[] memory names
    ) internal {
        if (!mintOpen) revert MintNotOpen();
        if (block.timestamp > auth.deadline) revert InvalidAuth();
        if (auth.wallet != msg.sender || auth.quantity == 0) revert InvalidAuth();
        if (auth.quantity != auth.freeQuantity + auth.paidQuantity) revert InvalidAuth();
        if (auth.nonce != nonces[msg.sender]) revert InvalidAuth();
        if (totalMinted + auth.quantity > mintSupplyCap) revert SoldOut();
        if (auth.slots.length != auth.quantity) revert InvalidSlots();
        if (names.length != 0 && names.length != auth.quantity) revert TokenNameCountMismatch();
        if (auth.slotsHash != keccak256(abi.encode(auth.slots))) revert InvalidSlots();

        for (uint256 index = 0; index < auth.quantity; index += 1) {
            uint256 slot = auth.slots[index];
            if (slot >= mintSupplyCap) revert InvalidSlots();
            if (slotMinted[slot]) revert SlotAlreadyMinted();
            for (uint256 previous = 0; previous < index; previous += 1) {
                if (auth.slots[previous] == slot) revert SlotAlreadyMinted();
            }
        }

        uint256 freeAvailable = FREE_PER_WALLET > freeMintCount[msg.sender]
            ? FREE_PER_WALLET - freeMintCount[msg.sender]
            : 0;
        if (auth.freeQuantity > freeAvailable) revert InvalidAuth();
        if (auth.freeQuantity > 0 && auth.slots[0] < REGAL_SLOT_COUNT) revert InvalidAuth();
        if (auth.freeQuantity > 0 && auth.coordinateHash == bytes32(0)) revert InvalidAuth();
        ...
        if (ECDSA.recover(digest, signature) != signer) revert InvalidAuth();

        nonces[msg.sender] = auth.nonce + 1;
        walletMintCount[msg.sender] += auth.quantity;

        for (uint256 index = 0; index < auth.quantity; index += 1) {
            uint256 slot = auth.slots[index];
            slotMinted[slot] = true;
            totalMinted += 1;
            tokenSlot[totalMinted] = slot;
            _safeMint(msg.sender, totalMinted);
            if (names.length != 0) {
                _setTokenName(totalMinted, names[index]);
            }
        }

        if (msg.value != 0) {
            (bool ok,) = fundsReceiver.call{value: msg.value}("");
            if (!ok) revert TransferFailed();
        }
    }

    function _requiredPayment(MintAuth calldata auth) internal view returns (uint256 total) {
        if (auth.slots.length != auth.quantity) revert InvalidSlots();
        for (uint256 index = 0; index < auth.slots.length; index += 1) {
            total += priceForSlot(auth.slots[index]);
        }
    }

    function _setTokenName(uint256 tokenId, string memory nextName) internal {
        if (bytes(nextName).length > MAX_TOKEN_NAME_BYTES) revert InvalidTokenName();
        tokenNames[tokenId] = nextName;
        emit TokenNameUpdated(tokenId, nextName);
    }
