// ===== FILE: src/factory/HaFactorySvg.sol =====
...
import {UriStorageSvg} from "../nft/UriStorageSvg.sol";
import {IHaFactory} from "./IHaFactory.sol";

contract HaFactorySvg {
    /// @dev Struct to store the parameters for the SVG
    struct SvgParams {
        string description;
        string svgPrefix;
        uint24[] colors;
        UriStorageSvg.Attribute[] attributes;
        UriStorageSvg.Trait[][] traits;
    }
    ...
    constructor(address factory, address universalRouter) {
        ...
        URI_STORAGE_IMPLEMENTATION = address(new UriStorageSvg());
    }
     ...
     * @param svgParams The parameters for the SVG
    ...
    function create(
        ...
        UriStorageSvg(uriStorage).initialize(address(this));

        UriStorageSvg(uriStorage).setDescription(svgParams.description);
        UriStorageSvg(uriStorage).setSvgPrefix(svgParams.svgPrefix);
        UriStorageSvg(uriStorage).addColors(svgParams.colors);
        UriStorageSvg(uriStorage).addAttributes(svgParams.attributes);
        uint256[] memory traitIndexes = new uint256[](svgParams.traits.length);
        for(uint256 i = 0; i < svgParams.traits.length; i++) {
            traitIndexes[i] = i;
        }
        UriStorageSvg(uriStorage).addTraits(traitIndexes, svgParams.traits);
        ...
        UriStorageSvg(uriStorage).setNft(nft);
