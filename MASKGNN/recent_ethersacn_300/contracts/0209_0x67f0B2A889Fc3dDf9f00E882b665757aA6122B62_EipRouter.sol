...
import {EipHook} from "./EipHook.sol";

/// @notice Thin V4 routing helper. Users who prefer the PoolManager swap
/// path can route through here; EipHook also exposes plain buy/sell so the
/// router is optional.
contract EipRouter is IUnlockCallback {
    ...
    EipHook      public immutable hook;
    IERC20       public immutable token;
    PoolKey      public poolKey;

    constructor(IPoolManager _pm, EipHook _hook, PoolKey memory _key) {
        if (Currency.unwrap(_key.currency0) != address(0))             revert WrongPool();
        if (Currency.unwrap(_key.currency1) != address(_hook.token())) revert WrongPool();
        if (address(_key.hooks) != address(_hook))                     revert WrongPool();
        poolManager = _pm;
        hook        = _hook;
        token       = IERC20(address(_hook.token()));
        poolKey     = _key;
    }

    function buy(uint256 minTokensOut) external payable returns (uint256 tokensOut) {
        require(msg.value > 0, "no eth");
        bytes memory unlockData = abi.encode(true, msg.sender, msg.value, minTokensOut);
        bytes memory ret = poolManager.unlock(unlockData);
    ...
    function sell(uint256 tokensIn, uint256 minEthOut) external returns (uint256 ethOut) {
        ...
        require(token.transferFrom(msg.sender, address(this), tokensIn), "tokenIn failed");
        bytes memory unlockData = abi.encode(false, msg.sender, tokensIn, minEthOut);
        bytes memory ret = poolManager.unlock(unlockData);
    ...
    function unlockCallback(bytes calldata data) external override returns (bytes memory) {
        ...
        (bool isBuy, address swapper, uint256 amountIn, uint256 minOut) =
            abi.decode(data, (bool, address, uint256, uint256));

        PoolKey memory key = poolKey;
        bytes memory hookData = abi.encode(swapper);

        if (isBuy) {
            return _runBuy(key, hookData, swapper, amountIn, minOut);
        } else {
            return _runSell(key, hookData, swapper, amountIn, minOut);
        }
    }

    function _runBuy(PoolKey memory key, bytes memory hookData, address swapper, uint256 amountIn, uint256 minOut)
        internal returns (bytes memory)
    {
        ...
        BalanceDelta delta = poolManager.swap(
            ...
            SwapParams({zeroForOne: true, amountSpecified: -int256(amountIn), sqrtPriceLimitX96: MIN_SQRT_LIMIT}),
            hookData
        );
        ...
        if (tokensOut < minOut) revert InsufficientOutput();

        poolManager.take(key.currency1, swapper, tokensOut);

        int128 ethDelta = delta.amount0();
        if (ethDelta > 0) {
            // forge-lint: disable-next-line(unsafe-typecast)
            poolManager.take(key.currency0, swapper, uint256(uint128(ethDelta)));
        }
        return abi.encode(tokensOut);
    }

    function _runSell(PoolKey memory key, bytes memory hookData, address swapper, uint256 amountIn, uint256 minOut)
        internal returns (bytes memory)
    {
        poolManager.sync(key.currency1);
        require(token.transfer(address(poolManager), amountIn), "tokenIn xfer");
        // slither-disable-next-line unused-return
        poolManager.settle();
        ...
        BalanceDelta delta = poolManager.swap(
            ...
            SwapParams({
                ...
                sqrtPriceLimitX96: MAX_SQRT_LIMIT
            }),
            hookData
        );
        ...
        int128 ethDelta = delta.amount0();
        // forge-lint: disable-next-line(unsafe-typecast)
        uint256 ethOut = ethDelta > 0 ? uint256(uint128(ethDelta)) : 0;
        if (ethOut < minOut) revert InsufficientOutput();

        poolManager.take(key.currency0, swapper, ethOut);
...
// ===== FILE: flora/contracts/lib/v4-core/src/interfaces/callback/IUnlockCallback.sol =====
// SPDX-License-Identifier: MIT
