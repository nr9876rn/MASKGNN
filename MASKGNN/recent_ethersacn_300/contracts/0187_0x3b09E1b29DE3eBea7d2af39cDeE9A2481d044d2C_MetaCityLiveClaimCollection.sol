...
// ===== FILE: contracts/MetaCityLiveClaimCollection.sol =====
// SPDX-License-Identifier: MIT
...
contract MetaCityLiveClaimCollection is
    ERC721,
    ERC2981,
    Ownable,
    ReentrancyGuard,
    Pausable
{
    IERC721 public originalContract;

    string private baseMetadataURI;

    uint256 public claimFee;
    uint256 public maxClaimFee = 0.1 ether;
    ...
    constructor(
        ...
        address initialOriginalContract,
        string memory initialBaseURI,
        uint256 initialClaimFee,
        uint256 initialMinTokenId,
        uint256 initialMaxTokenId,
    ...
    {
        require(initialOriginalContract != address(0), "Invalid original contract");
        require(bytes(initialBaseURI).length > 0, "Invalid base URI");
        require(initialOwner != address(0), "Invalid owner");
        require(royaltyReceiver != address(0), "Invalid royalty receiver");
        require(initialClaimFee <= maxClaimFee, "Initial fee too high");
        require(initialMinTokenId <= initialMaxTokenId, "Invalid token range");
        require(royaltyFeeNumerator <= MAX_ROYALTY_BPS, "Royalty too high");

        originalContract = IERC721(initialOriginalContract);
        baseMetadataURI = initialBaseURI;
        claimFee = initialClaimFee;
        minTokenId = initialMinTokenId;
        maxTokenId = initialMaxTokenId;

        _setDefaultRoyalty(royaltyReceiver, royaltyFeeNumerator);
    }

    function claim(uint256 tokenId) external payable nonReentrant whenNotPaused {
        require(msg.value == claimFee, "Incorrect ETH amount");

        _claimToken(msg.sender, tokenId);
    }
    ...
    function batchClaim(
        ...
        require(msg.value == claimFee * tokenIds.length, "Incorrect ETH amount");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            _claimToken(msg.sender, tokenIds[i]);
        }
    }

    function _claimToken(address claimer, uint256 tokenId) internal {
        require(tokenId >= minTokenId && tokenId <= maxTokenId, "Token out of range");
        require(!claimed[tokenId], "Already claimed");
        require(originalContract.ownerOf(tokenId) == claimer, "Not original owner");

        claimed[tokenId] = true;

        _safeMint(claimer, tokenId);

        emit Claimed(claimer, tokenId);
    }
    ...
    function canClaim(
        ...
        try originalContract.ownerOf(tokenId) returns (address owner) {
            return owner == wallet;
        } catch {
            return false;
        }
    ...
    function originalOwnerOf(
        ...
        return originalContract.ownerOf(tokenId);
    }
    ...
    function setClaimFee(uint256 newFee) external onlyOwner {
        require(newFee <= maxClaimFee, "Fee too high");
    ...
    function setBaseURI(string calldata newBaseURI) external onlyOwner {
        ...
        string memory oldBaseURI = baseMetadataURI;
