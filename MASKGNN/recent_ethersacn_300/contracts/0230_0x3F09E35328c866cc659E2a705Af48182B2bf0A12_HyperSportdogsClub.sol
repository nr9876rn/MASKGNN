// ===== FILE: _openzeppelin/contracts/access/AccessControl.sol =====
...
// ===== FILE: contracts/HyperSportdogsClub.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;
...
contract HyperSportdogsClub is ERC721, AccessControl, ReentrancyGuard {
    // ─── Roles ─────────────────────────────────────────────────────────────
    ...
    Payee[7] private _payees;
    uint256 public totalReleased;

    mapping(address => uint256) private _walletToPayeeId;
    mapping(address => bool) public isPayee;

    // ─── Collection config ─────────────────────────────────────────────────
    // 9,999 total. 399 tokens airdropped to the airdrop charity wallet (IDs 1-399).
    uint256 public constant MAX_SUPPLY = 9999;
    uint256 public constant PREMINT_COUNT = 399;

    // Whitelist caps. The distinct-wallet count is an off-chain concern (driven by
    // the Merkle tree); on-chain we mirror the roster size, and the total-mint cap
    // is always wlMaxPerWallet × whitelistRosterSize (see maxWhitelistMints()).
    uint256 public wlMaxPerWallet = 10;
    uint256 public whitelistRosterSize = 300;

    // ─── Paid mint pointer ─────────────────────────────────────────────────
    //  Airdropped tokens occupy IDs 1-399.
    //  Whitelist + public mints occupy token IDs 400-9999.
    uint256 private _nextMintId = PREMINT_COUNT + 1; // 400

    // ─── Supply counters ───────────────────────────────────────────────────
    uint256 public totalWhitelistMinted;                 // total WL tokens minted (across all wallets)
    mapping(address => uint256) public wlMinted;          // per-wallet WL mint count
    ...
    // ─── Events ────────────────────────────────────────────────────────────
    event NFTMinted(address indexed minter, uint256 indexed tokenId, uint256 quantity, uint256 price, uint256 phase);
    ...
    event WlMaxPerWalletChanged(uint256 newMax);
    event WhitelistRosterSizeChanged(uint256 newSize);
    event BaseURIChanged(string newURI);
    event PaymentReleased(address indexed to, uint256 amount);
    event PayeeWalletChanged(uint256 indexed payeeId, address indexed oldWallet, address indexed newWallet);

    // ─── Errors ────────────────────────────────────────────────────────────
    error InvalidPhase();
    error ExceedsMaxSupply();
    error ExceedsWhitelistMints();
    error ExceedsPerWalletCap();
    ...
    ) ERC721("Hyper Sportdogs Club", "HSC") {
        whitelistPrice = _whitelistPrice;
        publicPrice = _publicPrice;
        _registerPayee(_walletA, 0, 5000); // 50% - HDS Collective, LLC (Client)
        _registerPayee(_walletB, 1,  500); // 5%  - Richmond Softworks, LLC (Developer)
        _registerPayee(_walletC, 2,  700); // 7%  - Gen Art Programmer
        _registerPayee(_walletD, 3, 1300); // 13% - CFDP Wallet (Charity, Discord, Promo)
        _registerPayee(_walletE, 4,  300); // 3%  - Originating Artist #1
        _registerPayee(_walletF, 5, 1000); // 10% - Infra & Mgmt
        _registerPayee(_walletG, 6, 1200); // 12% - Originating Artist Transferable #2

        // Airdrop tokens 1-399 to airdrop charity wallet
        for (uint256 i = 1; i <= PREMINT_COUNT; i++) {
            _mint(_airdropWallet, i);
        }

        _baseTokenURI = baseURI_;

        // Set up OWNER_ROLE as the role-admin of both roles
        _setRoleAdmin(ADMIN_ROLE, OWNER_ROLE);
        _setRoleAdmin(OWNER_ROLE, OWNER_ROLE);

        // Grant roles
        _grantRole(OWNER_ROLE, _walletA);
        _grantRole(ADMIN_ROLE, _walletB);
        _grantRole(ADMIN_ROLE, _walletF);

        // Start paused
        currentPhase = Phase.Paused;
    }

    function _registerPayee(address wallet, uint256 id, uint256 share) private {
        if (isPayee[wallet]) revert DuplicatePayee();
        _payees[id] = Payee(wallet, share, 0);
        isPayee[wallet] = true;
        _walletToPayeeId[wallet] = id;
    }
    ...
    /// @notice Total tokens minted (premint + paid)
    function totalSupply() public view returns (uint256) {
        return _nextMintId - 1;
    }

    /// @notice Total WL tokens that may be minted. Always wlMaxPerWallet × whitelistRosterSize.
    function maxWhitelistMints() public view returns (uint256) {
        return wlMaxPerWallet * whitelistRosterSize;
    }

    // ─── Minting ───────────────────────────────────────────────────────────

    /// @notice Whitelist mint — requires Merkle proof and payment.
    ///         Caller may mint up to wlMaxPerWallet total across calls.
    ///         Reverts once the total WL mint cap (maxWhitelistMints) is reached.
    function whitelistMint(
        ...
        uint256 already = wlMinted[msg.sender];
        if (already + quantity > wlMaxPerWallet) revert ExceedsPerWalletCap();

        if (totalWhitelistMinted + quantity > maxWhitelistMints()) revert ExceedsWhitelistMints();

        if (_nextMintId - 1 + quantity > MAX_SUPPLY) revert ExceedsMaxSupply();

        // Cache price before any external calls so the event reflects validated price.
        ...
        wlMinted[msg.sender] = already + quantity;
        totalWhitelistMinted += quantity;

        uint256 startTokenId = _nextMintId;
        for (uint256 i = 0; i < quantity; i++) {
            _mint(msg.sender, _nextMintId);
            _nextMintId++;
        }
        emit NFTMinted(msg.sender, startTokenId, quantity, price, 1);
    }

    /// @notice Public mint — open to anyone with payment.
    ///         Mints from the paid range (token IDs 100+).
    function publicMint(uint256 quantity) external payable nonReentrant {
        ...
        emit NFTMinted(msg.sender, startTokenId, quantity, price, 2);
    }

    // ─── Revenue distribution (pull-based) ─────────────────────────────────

    /// @notice Returns the amount of ETH a payee can currently withdraw
    function releasable(address account) public view returns (uint256) {
        if (!isPayee[account]) return 0;
        uint256 id = _walletToPayeeId[account];
        uint256 totalReceived = address(this).balance + totalReleased;
        uint256 owed = (totalReceived * _payees[id].share) / BPS_DENOMINATOR;
        return owed - _payees[id].released;
    }

    /// @notice Withdraw caller's accrued revenue share.
    function claimRevenueShare() external nonReentrant {
        if (!isPayee[msg.sender]) revert NotAPayee();

        uint256 payment = releasable(msg.sender);
        if (payment == 0) revert NothingToWithdraw();

        uint256 id = _walletToPayeeId[msg.sender];
        _payees[id].released += payment;
    ...
    // ─── Payee wallet management ───────────────────────────────────────────

    /// @notice Allows a payee to redirect future revenue to a new wallet
    function setPayeeWallet(address newWallet) external {
        if (!isPayee[msg.sender]) revert NotAPayee();
        if (newWallet == address(0)) revert ZeroAddress();
        if (isPayee[newWallet]) revert WalletAlreadyPayee();

        uint256 id = _walletToPayeeId[msg.sender];

        delete isPayee[msg.sender];
        delete _walletToPayeeId[msg.sender];

        _payees[id].wallet = newWallet;
        isPayee[newWallet] = true;
        _walletToPayeeId[newWallet] = id;

        emit PayeeWalletChanged(id, msg.sender, newWallet);
    }

    // ─── Payee view helpers ────────────────────────────────────────────────

    function payeeWallet(uint256 payeeId) external view returns (address) {
        return _payees[payeeId].wallet;
    }

    function payeeShare(uint256 payeeId) external view returns (uint256) {
        return _payees[payeeId].share;
    }

    function payeeReleased(uint256 payeeId) external view returns (uint256) {
        return _payees[payeeId].released;
    }

    // ─── Admin functions (admin or owner) ──────────────────────────────────

    function setWhitelistPrice(uint256 _price) external onlyAdminOrOwner {
        whitelistPrice = _price;
        emit WhitelistPriceChanged(_price);
    }

    function setPublicPrice(uint256 _price) external onlyAdminOrOwner {
        publicPrice = _price;
    ...
    function setWlMaxPerWallet(uint256 _max) external onlyAdminOrOwner {
        wlMaxPerWallet = _max;
        emit WlMaxPerWalletChanged(_max);
    }

    function setWhitelistRosterSize(uint256 _size) external onlyAdminOrOwner {
        whitelistRosterSize = _size;
        emit WhitelistRosterSizeChanged(_size);
    }

    function setPhase(Phase _phase) external onlyAdminOrOwner {
        currentPhase = _phase;
        emit PhaseChanged(_phase);
    }

    function setBaseURI(string calldata _uri) external onlyAdminOrOwner {
        _baseTokenURI = _uri;
