...
// ===== FILE: contracts/main/SatToken.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.26;
...
/**
 * @title  SatToken
 * @notice Minimal ERC20 deployed by SatFactory.
 *         Fixed 18 decimals. Total supply minted entirely to the deployer (factory).
 */
...
// ===== FILE: contracts/main/SatTokenDeployer.sol =====
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.26;
...
/**
 * @notice Deploys SatToken instances on behalf of SatFactory.
 *         Keeping token bytecode here (not in the factory) reduces factory size.
 */
contract SatTokenDeployer is ITokenDeployer {
    function deploy(
        string  calldata name,
        string  calldata symbol,
        uint256          supply,
        address          /* poolManager — unused */,
        address          /* devWallet   — unused */
    ) external override returns (address token) {
        token = address(new SatToken(name, symbol, supply, msg.sender));
    }
}
