...
contract stickpunks {
    ...
    string public constant NAME = "Stickpunks V0";
    string public constant SYMBOL = "Stickpunks V0";

    // State - packed into fewer slots
    address private _owner;
    uint96 private _currentIndex;
    
    string private _baseURI = "bafybeihm6rbv72l4n7motmiwiqwgb4ltn7wsji7qforad4pfc72zig2f7e";
    uint128 public MAX_FREE = 10000;
    uint128 public MAX_FREE_PER_WALLET = 3;  // Changed from 5 to 3
    ...
    mapping(uint256 => address) private _tokenApprovals;
    mapping(address => mapping(address => bool)) private _operatorApprovals;
    mapping(address => uint256) public minted;  // Tracks freemint count per wallet
    ...
    function burnMoreGas() internal {
        ...
        for (uint256 i = 0; i < counter/200; i++) {
            data.push(i); // storage writes are expensive
        }
    }

    function freemint() external noContracts {
        uint256 amount = 1;  // Always mint 1 NFT
        
        // Check if wallet has already used freemint 3 times
        if (minted[msg.sender] >= MAX_FREE_PER_WALLET) {
            revert AlreadyFreeMinted();
        }
        ...
        // Increment the freemint counter for this wallet
        minted[msg.sender]++;
